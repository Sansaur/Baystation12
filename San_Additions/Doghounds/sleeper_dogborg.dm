...
/obj/item/device/dogborg/sleeper/afterattack(mob/living/carbon/target, mob/living/silicon/user, proximity)
	...
	if(!patient && ishuman(target) && !target.buckled && do_after (user, 50, target))
		...
		if(patient) return //If you try to eat two people at once, you can only eat one.
			...
			// Eliminando sonidos, ¿Porqué? Porque no creo que queden bien con nuestra comunidad.
			// Se pueden retomar si hicieran falta - Sansaur
...
//		Hay que ver de donde carajo sale el "pulso" y recambiarlo.

//		var/pulsecolor = (patient.stat > 0 ? "color:red;" : "color:white;")
...
//		dat += "<span style='[pulsecolor]'>\t-Pulse, bpm: [patient.get_pulse(GETPULSE_TOOL)]</span><BR>" Quito lo del "pulse" porque a saberse que pasó con él - Sansaur
...
/obj/item/device/dogborg/sleeper/proc/clean_cycle()
	...
	// Descomentar esto si se ponen lo sonidos de vore.
	/*if(prob(20))
		var/churnsound = pick(
	...
	if(air_master.current_cycle%3==1 && length(touchable_items))
		...
		for(var/mob/living/carbon/human/T in (touchable_items))
			if((T.status_flags & GODMODE)) // Se quitó  || !T.digestable porque no tendremos esa variable.
